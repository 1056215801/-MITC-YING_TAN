 ...
 * @company mitesofor
 */
@RequestMapping(value = "/reportThingRepair")
@RestController
@Slf4j
@Api(tags = "报事报修")
public class ReportThingRepairController {
    ...
    @ApiOperation(value = "分页查询报事报修列表", notes = "输入参数：zoneId 分区id，buildingId 楼栋id，unitId 单元id，" +
            ...
            "  appointmentTimeStart 预约开始时间、appointmentTimeEnd 预约结束时间、gmtCreateStart 报修开始时间 gmtCreateEnd 报修结束时间" +
    ...
    public Result listPage(HttpServletRequest request, Integer zoneId, Integer buildingId, Integer unitId,
                           ...
                           String gmtCreateStart, String gmtCreateEnd, Integer pageNum, Integer pageSize) {
        String sessionId = CookieUtils.getSessionId(request);
        SysUser user = (SysUser) redisService.get(RedisConstant.SESSION_ID + sessionId);
        String communityCode = user.getCommunityCode();
        Page<ReportThingsRepair> reportThingsRepairs = reportThingsRepairService.listPage(communityCode,
                zoneId, buildingId, unitId, roomId,
                cellphone, status, appointmentTimeStart, appointmentTimeEnd, gmtCreateStart,
                gmtCreateEnd, maintainType, pageNum, pageSize);
