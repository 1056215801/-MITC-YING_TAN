...
/*    @ApiImplicitParams({
    ...
    @PostMapping("/update")
    public Result update(ClusterCommunity clusterCommunity,HttpServletRequest request) {
        ...
        ClusterCommunity community = clusterCommunityService.selectOne(wrapper);
        Integer id = community.getId();
        clusterCommunity.setId(id);
...
//          clusterCommunityService.insert(clusterCommunity);
    ...
    public Result save(String communityName, String provinceName, String provinceId,
        ...
        communityEntityWrapper.eq("city_name",cityName);
        communityEntityWrapper.eq("area_name",areaName);
        communityEntityWrapper.eq("community_name",communityName);
        List<ClusterCommunity> clusterCommunities = clusterCommunityService.selectList(communityEntityWrapper);
        if (clusterCommunities.size()>0){
            return Result.error("当前区县下小区名已经存在");
        }
