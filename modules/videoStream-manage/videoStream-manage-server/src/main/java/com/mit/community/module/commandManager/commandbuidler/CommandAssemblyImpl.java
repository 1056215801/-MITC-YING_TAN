	...
	public String assembly(Map<String, String> paramMap) {
		try {
			if (paramMap.containsKey("ffmpegPath")) {
				...
				// -i：输入流地址或者文件绝对地址
				StringBuilder comm = new StringBuilder(ffmpegPath + " -re  -rtsp_transport tcp -i ");
				...
				if (paramMap.containsKey("input") && paramMap.containsKey("output") && paramMap.containsKey("appName")
					...
					String vprofile=(String)paramMap.get("vprofile");
					// 默认h264解码
					...
					if ("0".equals(twoPart)) {
						...
						if (paramMap.containsKey("fmt")) {
							...
							comm.append(" -f " + fmt);
							comm.append("  -vcodec "+codec+" -vprofile  "+vprofile);
						}
						// -r :帧率，默认25；-g :帧间隔
